...
function v(e) {
  ...
  (!e.layers || e.layers.length < 0) && p("no basemap defined, please define a basemap in the configuration"), e.layers.forEach((o) => {
    (!o.name || n.BASEMAP_PROVIDERS[o.name] !== void 0) && p("basemap names need to be defined and unique: " + o.name), n.BASEMAP_PROVIDERS[o.name] = b(d(t, o));
  });
}
function y(e) {
  let t = d(n.WMS_DEFAULTS, e.defaults);
  e.layers && e.layers.forEach((o) => {
    (!o.name || n.WMS_PROVIDERS[o.name] !== void 0) && p("wms names need to be defined and unique: " + o.name), n.WMS_PROVIDERS[o.name] = w(d(t, o));
  });
}
function k(e) {
  n.GEOCODER.lookupUrl = e.lookupUrl, n.GEOCODER.suggestUrl = e.suggestUrl, n.GEOCODER.placeholder = e.placeholder;
}
function R(e) {
  n.MAP = d(n.MAP, e);
}
...
function w(e) {
  ...
  if (t > -1) {
    let o = e.url.indexOf("}");
    e.url.slice(t + 1, o).toLowerCase() === "workspacename" ? e.url = e.url.slice(0, t) + e.workSpaceName + e.url.slice(o + 1, -1) : p("only workspacename templates are supported for now");
  }
  return e;
}
function A(e) {
...
function h(e) {
  ...
  const t = e.split("(")[1].split(")")[0], o = parseFloat(t.split(" ")[0]), s = parseFloat(t.split(" ")[1]);
  return {
    type: "Point",
    coordinates: [o, s]
  };
...
i.doLookupRequest = function(e) {
  return S(`${this.lookupUrl}id=${encodeURIComponent(e)}`).then((t) => {
    ...
    return o.centroide_ll = h(o.centroide_ll), o.centroide_rd = h(o.centroide_rd), o;
  });
};
i.createControl = function(e, t) {
  this.zoomTo = e, this.map = t;
  const o = document.createElement("div"), s = document.createElement("form"), r = document.createElement("input"), c = document.createElement("button"), m = document.createElement("div");
  return u(o, n.CLASSNAMES.geocoderContainer), u(s, n.CLASSNAMES.geocoderSearch), o.addEventListener("click", (a) => a.stopPropagation()), o.addEventListener("dblclick", (a) => a.stopPropagation()), r.id = "nlmaps-geocoder-control-input", r.placeholder = i.placeholder, r.setAttribute("aria-label", i.placeholder), r.setAttribute("type", "text"), r.setAttribute("autocapitalize", "off"), r.setAttribute("autocomplete", "off"), r.setAttribute("autocorrect", "off"), r.setAttribute("spellcheck", "false"), r.addEventListener("keydown", (a) => {
    let f = this.resultList;
    this.resultList.length > 0 && ((a.code === "ArrowDown" || a.keyCode === 40) && (this.selectedResult < this.resultList.length - 1 && this.selectedResult++, this.showLookupResult(f[this.selectedResult])), (a.code === "ArrowUp" || a.keyCode === 38) && (this.selectedResult > 0 && this.selectedResult--, this.showLookupResult(f[this.selectedResult])), a.code === "Escape" && this.clearSuggestResults(!0));
  }), r.addEventListener("input", (a) => {
    this.suggest(a.target.value);
  }), r.addEventListener("focus", (a) => {
    this.suggest(a.target.value);
  }), c.setAttribute("type", "submit"), s.addEventListener("submit", (a) => {
    a.preventDefault(), this.resultList.length > 0 && this.lookup(this.resultList[this.selectedResult < 0 ? 0 : this.selectedResult].id);
  }), c.setAttribute("aria-label", i.placeholder), u(c, n.CLASSNAMES.geocoderButton), m.id = "nlmaps-geocoder-control-results", u(m, n.CLASSNAMES.geocoderResultList), m.classList.add("nlmaps-hidden"), o.appendChild(s), s.appendChild(r), s.appendChild(c), o.appendChild(m), o;
};
...
i.showLookupResult = function(e) {
  ...
  Array.prototype.map.call(t, (s) => s.classList.remove(n.CLASSNAMES.geocoderResultSelected));
  let o = document.getElementById(e.id);
  o && o.classList.add(n.CLASSNAMES.geocoderResultSelected), document.getElementById("nlmaps-geocoder-control-input").value = e.weergavenaam;
};
function u(e, t) {
  t.forEach((o) => {
    e.classList.add(o);
...
i.showSuggestResults = function(e) {
  if (this.clearSuggestResults(), e.length > 0) {
    ...
    e.forEach((o) => {
      const s = document.createElement("li"), r = document.createElement("a");
      r.innerHTML = o.weergavenaam, r.id = o.id, u(r, n.CLASSNAMES.geocoderResultItem), r.setAttribute("href", "#"), r.addEventListener("click", (c) => {
...
function P(e) {
  if (e in n.BASEMAP_PROVIDERS) {
    ...
    console.error("NL Maps error: You asked for a style which does not exist! Available styles: " + Object.keys(n.BASEMAP_PROVIDERS).join(", "));
}
function _(e, t) {
  let o;
  return e in n.WMS_PROVIDERS ? (o = n.WMS_PROVIDERS[e], o.deprecated && console && console.warn && console.warn(e + " is a deprecated wms; it will be redirected to its replacement. For performance improvements, please change your reference.")) : (o = Object.assign({}, n.WMS_DEFAULTS, t), console.log(
    "NL Maps: You asked for a wms which does not exist! Available wmses: " + Object.keys(n.WMS_PROVIDERS).join(", ") + ". Provide an options object to make your own WMS."
  )), o;
}
function O() {
  let e = M(), t = L.latLng(e[0], e[1]), o = L.latLng(e[2], e[3]);
  return L.latLngBounds(t, o);
}
typeof L < "u" && typeof L == "object" && (L.NlmapsBgLayer = L.TileLayer.extend({
  initialize: function(e = "standaard", t) {
    const o = P(e), s = L.Util.extend({}, t, {
      minZoom: o.minZoom,
      maxZoom: o.maxZoom,
      scheme: "xyz",
  ...
  initialize: function(e = "", t) {
    const o = _(e, t), s = o.url, r = L.Util.extend({}, t, {
      layers: o.layerName,
      maxZoom: 24,
      minZoom: 1,
      styles: o.styleName,
  ...
  onAdd: function(e) {
    ...
    return L.DomEvent.on(
      ...
      L.DomUtil.removeClass(t, "started"), L.DomUtil.addClass(t, "has-position"), s(r);
    }), t;
  },
  onRemove: function(e) {
    return e;
